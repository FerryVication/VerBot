...
var handler = async (m, {
 ...
 command
    ...
    try {
        ...
        else if (command == 'pentol' || command == 'spentol') {
        	const res = `https://api.betabotz.eu.org/api/sticker/pentol?apikey=${lann}`
        		var stiker = await sticker5(res, { packname })
        		await conn.sendFile(m.chat, stiker, 'emror.webp', '', m)
        }
...
handler.command = handler.help = ['dinokuning', 'patrick', 'spongebob', 'doge', 'manusialidi', 'sdino', 'spatrick', 'sspongebob', 'sdoge', 'smanusialidi','pentol','spentol']
